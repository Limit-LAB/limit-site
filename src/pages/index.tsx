...
const IndexPage = definePage(() => {
  ...
  useEffect(() => {
    if (window) {
      setPositionPercentage(Math.round((scrollY / (height - height * 0.4)) * 100))
    }
  }, [scrollY, height])
      ...
      <div className="mt-10">
        <div className="w-full group pt-10 sm:pt-40 mt-10">
          <div className="flex h-24 rounded-3xl flex-row items-center justify-around" style={{ background: "linear-gradient(188.88deg, #0080FB 6.75%, #153CA9 124.65%)" }}>
            <p className={`text-10xl relative`} style={{ left: positionPercentage * 5 - 300 || 0, bottom: positionPercentage * 5 - 300 || 0 }}>🚀
            </p>
            <p className={
              "text-white/70 font-extrabold text-xl md:text-4xl p-4" +
              " transform duration-150 ease-linear relative group-hover:(translate-x-6 text-white)"
            }> WE STARTUP, IN PROGRESS NOW!</p>
          </div>
        </div>
        <h2 className="mt-36 font-bold text-3xl sm:mb-18 <sm:mb-12 text-center">Constituents</h2>
        <div className="mt-10  md:h-80 rounded-3xl md:mb-0 shadow shadow-xl group bg-white overflow-hidden hover:cursor-pointer flex flex-col md:flex-row justify-between items-center" >
          <div className="p-10">
            ...
            <Button text="Github Repo" variant='primary' />
          </div>
